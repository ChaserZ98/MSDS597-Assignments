...
```{r}
treasureIslandText <- gutenberg_download(120)
treasureIslandText
```
...
```{r}
kidnappedText <- gutenberg_download(421)
kidnappedText
```
...
## 6. Plot the sentiment for the two books using the bing lexicon, using 100 words as the unit of length.
...
treasureIslandText %>%
  ...
  mutate(index = row_number() %/% unitLength) %>%
